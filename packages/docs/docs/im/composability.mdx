...
hackblock.space is built for composability. As we saw in [Getting Started](./getting_started),
you can easily create token gated chatrooms and embed them anywhere.

hackblock.space goes one step further. Because a chatroom is just transaction history
