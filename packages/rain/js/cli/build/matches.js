...
const raindrops_cli_1 = require("@raindrops-protocol/raindrops-cli");
const { loadWalletKey } = sol_command_1.Wallet;
const { PDA } = raindrops_cli_1.Utils;
var MatchesState = raindrops_cli_1.State.Matches;
programCommand("create_match")
    ...
    const anchorProgram = await (0, raindrops_cli_1.getMatchesProgram)(walletKeyPair, env, rpcUrl);
    ...
    loglevel_1.default.info("Namespaces:", u.namespaces
        ? u.namespaces.map((u) => {
            if (!u.namespace.equals(anchor_1.web3.SystemProgram.programId))
                loglevel_1.default.info(`--> ${raindrops_cli_1.State.InheritanceState[u.inherited]} ${u.namespace.toBase58()} Indexed: ${u.indexed}`);
        })
        : "Not Set");
